    ...
    <Link href={`/products/${product.id}`}>
      <article className="group cursor-pointer relative">
        ...
        <div className="relative overflow-hidden rounded-2xl md:rounded-4xl bg-[#F5F3FF] transition-all duration-500 shadow-sm group-hover:shadow-lg aspect-4/5">
          <Image
            src={product.image || product.image_url || "https://images.unsplash.com/photo-1523381210434-271e8be1f52b?q=80&w=400&h=500&auto=format&fit=crop"}
            alt={product.name}
            fill
          ...
          >
            {isOutOfStock ? "Sold Out" : isLowStock ? `Only ${product.stock} Left` : isFewLeft ? "Limited Edition" : "Exclusive"}
          </span>
          {isOutOfStock && (
        ...
        <div className="mt-4">
          ...
          <div className="flex items-center gap-3 mt-2">
            <span className={`font-bold text-base tracking-tight ${isOutOfStock ? "text-[#4C1D95]/40 line-through" : "text-[#4C1D95]"}`}>
              ₹{Number(product.price).toLocaleString()}
            </span>
            {isLowStock && (
              <span className="text-orange-500 text-[9px] font-bold italic animate-bounce">Selling Fast!</span>
            )}
          </div>
        </div>
      </article>
