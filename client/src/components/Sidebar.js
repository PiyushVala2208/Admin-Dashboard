    ...
    <>
      ...
      >
        <div className="flex items-center justify-between p-5 h-20 border-b border-slate-800/50">
          ...
          >
            {isMobileOpen ? <X size={22} /> : isCollapsed ? <PanelLeftOpen size={22} /> : <PanelLeftClose size={22} />}
          </button>
        </div>

        <nav className="flex-1 px-3 py-6 space-y-1.5 overflow-y-auto no-scrollbar">
          {menuItems.map((item) => {
            const hasSubActive = item.submenu?.some((sub) => isActive(sub.path));
            ...
            return (
              <div key={item.name} className="relative">
                {!item.submenu ? (
                  ...
                  >
                    <span className={`${isActive(item.path) ? "scale-110" : "group-hover:scale-110"} transition-transform`}>
                      {item.icon}
                    </span>
                    {(!isCollapsed || isMobileOpen) && <span className="font-medium">{item.name}</span>}
                  </Link>
                ) : (
