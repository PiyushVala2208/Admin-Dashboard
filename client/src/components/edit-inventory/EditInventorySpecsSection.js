...
// Props: { isLoadingSpecs, categoryId, mappedAttributes, fixedSpecificationAttributes, specValues, onSpecChange }
function EditInventorySpecsSection({
  isLoadingSpecs,
  categoryId,
  mappedAttributes,
    ...
    <section className="rounded-[1.8rem] border border-slate-200 bg-white p-6 shadow-sm md:p-7">
      ...
      {isLoadingSpecs ? (
        <div className="space-y-3">
          ...
          <div className="h-11 w-full animate-pulse rounded-xl bg-slate-100" />
          <div className="h-11 w-4/5 animate-pulse rounded-xl bg-slate-100" />
        </div>
      ) : !categoryId ? (
...
export default memo(EditInventorySpecsSection);
